...
int main(void) {
    ...
    if (thbc_server_start(&server) < 0) {
        perror("main: thbc_start_server");
        return EXIT_FAILURE;
    }

    thbc_viewer_state_t viewer_state = empty_thbc_viewer_state_t;
    if (thbc_viewer_init(&viewer_state, &server) < 0) {
        perror("main: thbc_init_viewer");
        return EXIT_FAILURE;
    }

    while (viewer_state.running && server.http_server.running) 
        thbc_viewer_update(&viewer_state, &server);
    thbc_viewer_close(&viewer_state, &server);

    thbc_server_stop(&server);
    return EXIT_SUCCESS;
